...
class SmoothAP(torch.nn.Module):
    ...
    def forward(self, preds,labels):
        ...
        calculated_idx_list = torch.zeros(self.batch_size)

        for i in range(self.batch_size):
            if calculated_idx_list[i] > 0.5:
                continue
            pos_idxs = labels == labels[i]
            calculated_idx_list[pos_idxs] = 1
            idx = all_idx[pos_idxs].cuda().detach()

            pos_mask = 1.0 - torch.eye(len(idx))
            pos_mask = pos_mask.unsqueeze(dim=0).repeat(len(idx), 1, 1)

            sim_pos = compute_aff(preds[idx])
            sim_pos_repeat = sim_pos.unsqueeze(dim=1).repeat(1, len(idx), 1)
            ...
            pos_divide = torch.sum(
                sim_pos_rk / (torch.index_select(torch.index_select(sim_all_rk, 0, idx), 1, idx)))
            ap = ap + ((pos_divide / len(idx)) / self.batch_size)
