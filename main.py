...
for epoch in range(opt.n_epochs):
    ...
    for i,out in enumerate(data_iterator):
        ...
        loss, loss_rank, loss_kd      = criterion(**loss_args)

        # print('loss:', loss, 'rank:', loss_rank, 'kd:', loss_kd)
        ...
        loss_rank_collect.append(loss_rank.item())
        loss_kd_collect.append(loss_kd.item())

        ###
        iter_count += 1

        if i==len(dataloaders['training'])-1: data_iterator.set_description('Epoch (Train) {0}: Mean Loss [{1:.4f}] Mean Rank-Loss [{1:.4f}] Mean KD-Loss [{1:.4f}]'
                                                                            .format(epoch, np.mean(loss_collect), np.mean(loss_rank_collect), np.mean(loss_kd_collect)))
    ...
    result_metrics = {'loss': np.mean(loss_collect), 'loss_rank': np.mean(loss_rank_collect), 'loss_kd': np.mean(loss_kd_collect)}
